...
const CareerOpportunities = () => {
  ...
  const onFindJobs = async () => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:8000/api/find-jobs/", formData);
  ...
  return (
    <Container maxWidth="md" className="signin-container">
      <ToastContainer />
      <div className="signin-form-container">
        <Typography variant="h4" className="signin-header">Career Opportunities 🌐</Typography>
        <Typography variant="body1" className="signin-subheader">Find job opportunities based on your skills and qualifications</Typography>

        <Grid container spacing={3} className="signin-form">
          <Grid item xs={12}>
          ...
          <Grid item xs={12}>
            ...
            {jobTypes.map((type, index) => (
              <Chip key={index} label={type} onClick={() => handleInputChange("jobType", type)} className={`chip ${formData.jobType === type ? "selected" : ""}`} />
            ))}
          </Grid>
          <Grid item xs={12}>
        ...
        <Box mt={4}>
          <Button variant="contained" color="primary" fullWidth size="large" className="signin-button" onClick={onFindJobs}>Find Jobs</Button>
        </Box>
      </div>
    </Container>
  );
